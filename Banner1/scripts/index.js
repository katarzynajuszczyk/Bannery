var animateUs = (function() {
  ...
  if(!Modernizr.csstransitions) {
    ...
    var background = myjQ('.mw-banner-background');
    var banckgroundWidth = background.width();
    var backgroundOffset = -(banckgroundWidth/2);
    
    background.css({ scale: 1.1, x: backgroundOffset });
    ...
    setInterval(function(){
      background.transition({
            scale: 1,
            x: Math.floor(Math.random() * (backgroundOffset)),
            duration: '6000',
            easing: 'linear'
      }).transition({
            scale: 1.1,
